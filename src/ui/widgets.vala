...
namespace Synapse.Gui
{
  ...
  public class CloneWidget : Gtk.Widget
  {
    ...
    {
	  clone.get_preferred_size (out req, out req);
    }
  }
  ...
  public class SmartLabel : Gtk.Misc
  {
    ...
    public bool expose_event (Gdk.EventExpose event)
    {
      ...
      ctx.translate (allocation.x + this.xpad, allocation.y + this.ypad);
  ...
  public class SchemaContainer: Gtk.Container
  {
    ...
    public override void size_allocate (Gtk.Allocation allocation)
    {
      ...
      foreach (Widget child in children)
      {
        ...
        if (alloc.length <= i)
        {
		  a.y = a.y = a.width = a.height = 0;
        }
        else
        {
		  a.x = x + alloc[i].x * this._scale_size / 100;
		  a.y = y + alloc[i].y * this._scale_size / 100;
		  a.width = alloc[i].width * this._scale_size / 100;
		  a.height = alloc[i].height * this._scale_size / 100;
        }
        if (rtl) a.x = 2 * allocation.x + allocation.width - a.x - a.width;
        // FIXME: child.size_allocate (a);
        i++;
      }
    }
  }
  ...
  public class SelectionContainer: Gtk.Container
  {
    ...
    public void size_request (out Requisition req)
    {
      req = {0, 0};
      if (active_child >= children.size) return;
      children.get (active_child).get_preferred_size (out req, out req);
    }
  }
  ...
  public class Throbber: Spinner
  {
    ...
    public bool expose_event (Gdk.EventExpose event)
    {
      if (this.active)
      {
        // FIXME: return base.expose_event (event);
		return true;
      }
      return true;
    }
  ...
  public class FakeInput: Gtk.Alignment
  {
    ...
    public bool expose_event (Gdk.EventExpose event)
    {
      if (draw_input)
      {
        ...
        double x = allocation.x      + this.left_padding,
               y = allocation.y      + this.top_padding,
        ...
        if (_focus_widget != null)
        {
          /*
           ...
           x1         x2         x3  y2
          */
		  Gtk.Allocation focus_widget_allocation;
		  _focus_widget.get_allocation (out focus_widget_allocation);
          double x1 = double.max (focus_widget_allocation.x, x),
                 x3 = double.min (focus_widget_allocation.x + focus_widget_allocation.width, x + w);
          double x2 = (x1 + x3) / 2.0;
          double y2 = y + h;
  ...
  public class MenuButton: FakeButton
  {
    ...
    public void size_allocate (Gtk.Allocation allocation)
    {
      Allocation alloc = {allocation.x, allocation.y, allocation.width, allocation.height};
      set_allocation (alloc);
    }
    ...
    public bool expose_event (Gdk.EventExpose event)
    {
      ...
      Pattern pat;
      pat = new Pattern.linear (allocation.x,
                                allocation.y,
                                allocation.x,
      ...
      size *= 0.5;
      ctx.set_source (pat);
      ctx.arc (allocation.x + allocation.width - SIZE * 2 - size,
               allocation.y + size,
               size, 0, Math.PI * 2);
      ctx.fill ();
  ...
  public class HTextSelector : EventBox
  {
    ...
    private bool update_current_offset ()
    {
      ...
      draw_offset = allocation.width / 2 - txt.offset - txt.width / 2;
